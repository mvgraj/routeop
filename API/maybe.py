...
async def cluster_and_route(
    ...
    # ✅ Fix: Ensure all values are non-negative and valid
    ...
    for idx, row in df.iterrows():
        folium.CircleMarker([row['Latitude'], row['Longitude']], radius=5).add_to(cluster_map)
    for idx, row in centroids.iterrows():
        folium.Marker([row['Latitude'], row['Longitude']], icon=folium.Icon(color="red")).add_to(cluster_map)
    ...
    for idx, (group_indices, total_students, bus_size) in enumerate(assigned_routes):
        ...
        url = f"http://localhost:5000/trip/v1/driving/{';'.join(coord_strs)}?source=first&destination=last"
        response = requests.get(url)
        ...
        folium.PolyLine(polyline.decode(geometry), color="blue").add_to(route_map)
