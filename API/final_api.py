...
def get_osrm_table_chunked(coord_list, max_chunk=50, retry=3):
    ...
    for i in tqdm(range(0, n, max_chunk), desc="Row chunks"):
        for j in range(0, n, max_chunk):
            ...
            url = (
                f"http://13.235.63.164:3003/table/v1/driving/{coord_str}"
...
async def cluster_and_route(
    ...
    for idx, (group_indices, total_students, bus_size) in enumerate(assigned_routes):
        ...
        url = f"http://13.235.63.164:3003/trip/v1/driving/{';'.join(coord_strs)}?source=first&destination=last"
        response = requests.get(url)
        ...
        fg = FeatureGroup(name=f"Bus {idx+1}")

        folium.PolyLine(decoded_coords, color="blue", weight=4, opacity=0.7).add_to(fg)
    ...
    return {
        ...
        "trip_id": trip_id,
        "parameters": metadata[trip_id]['parameters'],
        "statistics": metadata[trip_id]['statistics'],
        "files": metadata[trip_id]['files']
    }


@app.get("/download_file/{trip_id}/{filename:path}")
def download_file(trip_id: str, filename: str):
    file_path = os.path.join("output", trip_id, filename)

    if not os.path.exists(file_path):
        return JSONResponse(status_code=404, content={"detail": "File not found"})

    if filename.endswith(".html"):
        return FileResponse(file_path, media_type="text/html", filename=os.path.basename(file_path),
